...
/**
 * PATCH /api/gear/:id/comments/:commentId
 *
 * Edits a comment's body. Only the comment author may edit — RLS UPDATE
 * policy enforces `auth.uid() = user_id`. The body length is also
 * re-checked on the policy level (1..2000 chars).
 *
 * 404 when the comment doesn't exist OR RLS blocks the UPDATE (i.e. the
 * caller is not the author). Same status either way so we don't leak the
 * row's existence to non-authors.
 */
